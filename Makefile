...
isort_check: ## check that isort has been run
	isort --check-only -rc estore_proj/

isort: ## run isort to sort imports in all Python files
	isort --recursive --atomic estore_proj/

style: ## run Python style checker
	pylint --rcfile=pylintrc estore_proj *.py
...
pii_check: ## check for PII annotations on all Django models
	DJANGO_SETTINGS_MODULE=estore_proj.settings.test \
...
dummy_translations: ## generate dummy translation (.po) files
	cd estore_proj && i18n_tool dummy
...
open-devstack: ## open a shell on the server started by start-devstack
	docker exec -it estore_proj /edx/app/estore_proj/devstack.sh open

pkg-devstack: ## build the estore_proj image from the latest configuration and code
	docker build -t estore_proj:latest -f docker/build/estore_proj/Dockerfile git://github.com/edx/configuration

detect_changed_source_translations: ## check if translation files are up-to-date
	cd estore_proj && i18n_tool changed

validate_translations: fake_translations detect_changed_source_translations ## install fake translations and check if translation files are up-to-date

docker_build:
	docker build . -f Dockerfile -t openedx/estore_proj
	docker build . -f Dockerfile --target newrelic -t openedx/estore_proj:latest-newrelic

travis_docker_tag: docker_build
	docker tag openedx/estore_proj openedx/estore_proj:$$TRAVIS_COMMIT
	docker tag openedx/estore_proj:latest-newrelic openedx/estore_proj:$$TRAVIS_COMMIT-newrelic

travis_docker_auth:
	echo "$$DOCKER_PASSWORD" | docker login -u "$$DOCKER_USERNAME" --password-stdin

travis_docker_push: travis_docker_tag travis_docker_auth ## push to docker hub
	docker push 'openedx/estore_proj:latest'
	docker push "openedx/estore_proj:$$TRAVIS_COMMIT"
	docker push 'openedx/estore_proj:latest-newrelic'
	docker push "openedx/estore_proj:$$TRAVIS_COMMIT-newrelic"
...
app-shell: # Run the app shell as root
	docker exec -u 0 -it Estore.app bash

db-shell: # Run the app shell as root, enter the app's database
	docker exec -u 0 -it Estore.db mysql -u root Estore
